---
description: Alias for /pipeline; create a complete kids nonfiction picture book with editor-gated phases
agent: pipeline
subtask: true
---
...
Run the `pipeline` subagent for the provided topic and target age. Treat this command as an alias for `/pipeline`, not as a separate manual workflow.

If the user provides build preferences such as template, seed, or strict mode, pass them through to the builder phase when reached. Default to strict builds in the full pipeline unless the user explicitly requests non-strict mode.
